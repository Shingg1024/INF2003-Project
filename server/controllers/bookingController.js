...
exports.getHostelCompleted = (req, res) => {
    db.getConnection((err, connection) => {
        ...
        const query = "SELECT bh.booking_hostel_id, h.hostel_id, h.hostel_name, DATE_FORMAT(bh.date_start, '%Y-%m-%d') AS date_start, DATE_FORMAT(bh.date_end, '%Y-%m-%d') AS date_end FROM hostel AS h JOIN booking_hostel AS bh ON h.hostel_id = bh.hostel_id WHERE bh.date_start < CURDATE() AND bh.user_id = ? ORDER BY bh.date_start DESC;";
        connection.query(query, [id], (err, result) => {
            try {
                if (err) {
                    console.log(err);
...
exports.getRestaurantCompleted = (req, res) => {
    db.getConnection((err, connection) => {
        ...
        const query = "SELECT br.booking_restaurant_id ,r.restaurant_id, r.restaurant_name, DATE_FORMAT(br.date, '%Y-%m-%d') AS date, br.time FROM restaurant AS r JOIN booking_restaurant AS br ON r.restaurant_id = br.restaurant_id WHERE br.date < CURDATE() AND br.user_id = ? ORDER BY br.date DESC;";
        connection.query(query, [id], (err, result) => {
            try {
                if (err) {
                    console.log(err);
